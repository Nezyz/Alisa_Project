...
def main_dialog(res, req):
    global current_status, current_dialog, Session_data, dialog_sport, flag, flag_end_football, \
        ...
        flag_leo, flag_info_ney, topic_alisa, heads_or_tails
    ...
    if current_dialog == "start":
        ...
        if current_status == "start_question" and req['request']['original_utterance'].lower() not in \
                ['давай о спорте.', 'просто поболтать.', 'вопросы по городам.', 'покажи города.',
                 'не знаю, выбери сама.', 'помощь.', "игра 'орёл&решка'"]:
            ...
            if req['request']['original_utterance'].lower() in ['помощь.']:
                res['response'][
                    'text'] = 'Я умею выполнять несколько команд:"Давай о спорте"-я могу поддержать небольшой' \
                              ...
                              '"Орёл&Решка" - Если тебе понадобитсяя помощь в решении, а у тебя не будет под рукой монетки, этот способ специально для тебя.'
                Session_data[user_id] = {
                    'suggests': [
                        "Давай о спорте.",
                        "Просто поболтать.",
    ...
    if current_status == "start_question":
        if req['request']['original_utterance'].lower() in \
                ["игра 'орёл&решка'", "игра 'орёл&решка'."]:
            res['response']['text'] = 'Хорошо, напиши "Брось монетку", и я скажу тебе, что выпало.'
            current_status = "random_play"
            Session_data[user_id] = {
                'suggests': [
                    "Брось монетку.",
                    "Не надо.",
            ...
            res['response']['buttons'] = get_suggests(user_id)
            return
    if current_status == "random_play":
        if req['request']['original_utterance'].lower() in \
                ["брось монетку", "брось монетку."]:
            result = random.choice(heads_or_tails)
            res['response']['text'] = 'Монетка показала: ' + result
        if req['request']['original_utterance'].lower() in \
                ["хватит", "хватит."]:
            res['response']['text'] = 'Хорошо. Была рада помочь.'
            current_dialog = "start"
            current_status = "start_question"
            Session_data[user_id] = {
                'suggests': [
        ...
        if req['request']['original_utterance'].lower() in \
                ["не надо.", "не надо"]:
            res['response']['text'] = 'Тогда выбирай тему.'
            current_dialog = "start"
            current_status = "start_question"
            Session_data[user_id] = {
                'suggests': [
            ...
            res['response']['buttons'] = get_suggests(user_id)
            return
        if req['request']['original_utterance'].lower() not in \
                ["брось монетку", "брось монетку.", "не надо.", "не надо", "хватит", "хватит."]:
            res['response']['text'] = 'Я тебя не поняла, повтори ещё раз.'
